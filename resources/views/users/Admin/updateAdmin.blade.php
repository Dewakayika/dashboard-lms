        ...
        <div class="p-3 py-5">
            <div class="d-flex justify-content-between align-items-center ">
                <div class="d-flex flex-row align-items-center back"><i class="fa fa-long-arrow-left "></i>
                    <a href="{{ route('admin#adminProfile') }}">Back to home</a>
                </div>
            ...
            <form action="{{ route('admin#updateAdmin') }}" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-1 gap-4 mt-4">
                    <div class="w-full ">
                        ...
                        <input type="text" class="form-control" value="{{ old('name', $adminData->users->name) }}"
                            name="name">
                    </div>
                </div>
                        ...
                        <div class="form-check form-check-inline flex gap-10">
                            <div>
                                <input class="form-check-input" type="radio" name="gender" id="inlineRadio1"
                                    value="0" required="" @if ($adminData->users->gender == 0) @return checked @endif>
                                <label class="form-check-label" for="inlineRadio1" name="gender">Male</label>
                            </div>
                            <div>
                                <input class="form-check-input" type="radio" name="gender" id="inlineRadio2"
                                    value="1" required="" @if ($adminData->users->gender == 1) @return checked @endif>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 my-3">
                    <div class="">
                        ...
                        <input type="text" class="form-control"
                            value="{{ old('address', $adminData->users->address) }}" name="address">
                    </div>
                    <div class="">
                        <label for="fname">Phone Number:</label>
                        <input type="tel" maxlength="11" class="form-control" value="{{ old('phone', $adminData->users->phone) }}"
                            name="phone">
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-1 gap-4 my-3">
                    <div class="">
                        ...
                        <input type="text" class="form-control" value="{{ old('email', $adminData->users->email) }}"
                            name="email">
                    </div>
                </div>
