@section('title')
    Padma Community | Gallery
@endsection

@extends('users.Member.layouts.app')

@section('content')




@if ($errors->any())
    <div class="alert alert-danger">
        <ul>
            @foreach ($errors->all() as $error)
                <li>{{ $error }}</li>
            @endforeach
        </ul>
    </div>
@endif

@if ($isEmpty)
<div class="flex items-center justify-center h-screen">
    <div class="grid gap-4 w-60">
        <svg class="mx-auto" xmlns="http://www.w3.org/2000/svg" width="116" height="121" viewBox="0 0 116 121" fill="none">
        <path d="M0.206909 63.57C0.206909 31.7659 25.987 6.12817 57.6487 6.12817C89.2631 6.12817 115.079 31.7541 115.079 63.57C115.079 77.0648 110.43 89.4805 102.627 99.2755C91.8719 112.853 75.4363 121 57.6487 121C39.7426 121 23.4018 112.794 12.6582 99.2755C4.85538 89.4805 0.206909 77.0648 0.206909 63.57Z" fill="#EEF2FF" />
        <path d="M72.7942 0.600875L72.7942 0.600762L72.7836 0.599331C72.3256 0.537722 71.8622 0.5 71.3948 0.5H22.1643C17.1256 0.5 13.0403 4.56385 13.0403 9.58544V107.286C13.0403 112.308 17.1256 116.372 22.1643 116.372H93.1214C98.1725 116.372 102.245 112.308 102.245 107.286V29.4482C102.245 28.7591 102.17 28.0815 102.019 27.4162L102.019 27.416C101.615 25.6459 100.67 24.0014 99.2941 22.7574C99.2939 22.7572 99.2937 22.757 99.2934 22.7568L77.5462 2.89705C77.5461 2.89692 77.5459 2.89679 77.5458 2.89665C76.2103 1.66765 74.5591 0.876968 72.7942 0.600875Z" fill="white" stroke="#E5E7EB" />
        <circle cx="60.2069" cy="61" r="21.0256" fill="#EEF2FF" />
        <path d="M74.6786 46.1412L74.6783 46.1409C66.5737 38.0485 53.4531 38.0481 45.36 46.1412C37.2552 54.2341 37.2551 67.3666 45.3597 75.4596C53.4529 83.5649 66.5739 83.5645 74.6786 75.4599C82.7716 67.3669 82.7716 54.2342 74.6786 46.1412ZM79.4694 41.3508C90.2101 52.0918 90.2101 69.5093 79.4694 80.2502C68.7166 90.9914 51.3104 90.9915 40.5576 80.2504C29.8166 69.5095 29.8166 52.0916 40.5576 41.3506C51.3104 30.6096 68.7166 30.6097 79.4694 41.3508Z" stroke="#E5E7EB" />
        <path d="M83.2471 89.5237L76.8609 83.1309C78.9391 81.5058 80.8156 79.6106 82.345 77.6546L88.7306 84.0468L83.2471 89.5237Z" stroke="#E5E7EB" />
        <path d="M104.591 94.4971L104.59 94.4969L92.7346 82.653C92.7342 82.6525 92.7337 82.652 92.7332 82.6515C91.6965 81.6018 90.0076 81.6058 88.9629 82.6505L89.3089 82.9965L88.9629 82.6505L81.8573 89.7561C80.8213 90.7921 80.8248 92.4783 81.8549 93.5229L81.8573 93.5253L93.7157 105.384C96.713 108.381 101.593 108.381 104.591 105.384C107.6 102.375 107.6 97.5062 104.591 94.4971Z" fill="#A5B4FC" stroke="#818CF8" />
        <path d="M62.5493 65.6714C62.0645 65.6714 61.6626 65.2694 61.6626 64.7729C61.6626 62.7866 58.6595 62.7866 58.6595 64.7729C58.6595 65.2694 58.2576 65.6714 57.761 65.6714C57.2762 65.6714 56.8743 65.2694 56.8743 64.7729C56.8743 60.422 63.4478 60.4338 63.4478 64.7729C63.4478 65.2694 63.0458 65.6714 62.5493 65.6714Z" fill="#4F46E5" />
        <path d="M70.1752 58.0694H66.4628C65.9662 58.0694 65.5642 57.6675 65.5642 57.1709C65.5642 56.6862 65.9662 56.2842 66.4628 56.2842H70.1752C70.6717 56.2842 71.0737 56.6862 71.0737 57.1709C71.0737 57.6675 70.6717 58.0694 70.1752 58.0694Z" fill="#4F46E5" />
        <path d="M53.8596 58.0693H50.1472C49.6506 58.0693 49.2487 57.6673 49.2487 57.1708C49.2487 56.686 49.6506 56.2841 50.1472 56.2841H53.8596C54.3443 56.2841 54.7463 56.686 54.7463 57.1708C54.7463 57.6673 54.3443 58.0693 53.8596 58.0693Z" fill="#4F46E5" />
        <rect x="28.9248" y="16.3846" width="30.7692" height="2.05128" rx="1.02564" fill="#4F46E5" />
        <rect x="28.9248" y="100.487" width="41.0256" height="4.10256" rx="2.05128" fill="#A5B4FC" />
        <rect x="28.9248" y="22.5385" width="10.2564" height="2.05128" rx="1.02564" fill="#4F46E5" />
        <circle cx="42.2582" cy="23.5641" r="1.02564" fill="#4F46E5" />
        <circle cx="46.3607" cy="23.5641" r="1.02564" fill="#4F46E5" />
        <circle cx="50.4633" cy="23.5641" r="1.02564" fill="#4F46E5" />
        </svg>
        <div>
        <h2 class="text-center text-black text-base font-semibold leading-relaxed pb-1">No assignment available to vote</h2>
        <p class="text-center text-gray-600 text-sm font-normal leading-snug pb-4">Please wait until your batchmates <br> upload their assignments.</p>
        <div class="flex gap-3">
        </div>
        </div>
    </div>
</div>

@else

@foreach($submissionData as $courseName => $submissions)
    <div class="container mb-4 mt-4">
        <h1 class="text-xl font-bold mb-4">Assignment for {{ $courseName }}</h1>
        <hr>
    </div>
    <div class="container grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 sm:grid-cols-2 gap-3 mb-4">
    @foreach($submissions as $submission)
        <div class="relative flex flex-col bg-white shadow-sm border border-slate-200 rounded-lg w-auto">
            <a href="{{ asset($submission->submission_file) }}" target="_blank">
                <div class="relative h-56 m-2.5 overflow-hidden text-white rounded-md group">
                    <img src="{{asset($submission->thumbnail)}}" alt="card-image" class="transition-transform duration-300 transform group-hover:scale-110" />
                </div>
            </a>
            
            <div class="flex px-4 mb-4">
                <div class="w-full">
                    <h6 class="mb-0 text-slate-800 text-xl font-semibold">
                        
                        {{$submission->user->name}}
                    </h6>
                    <div class="flex items-center">
                        <div class="text-sm">
                            <a href="#" class="text-gray-600 font-regular leading-none hover:text-indigo-600">{{$submission->chapter_name}} | {{$submission->course_name}} </a>
                        </div>
                    </div>
                </div>

                @if(!$submission->alreadyVoted)
                <div class="absolute flex justify-center  items-center gap-2 bg-white px-3 py-1 rounded text-black right-4 top-4">
                    <i class="fa-solid fa-medal"></i>
                    <p class="font-semibold">{{ $submission->votes->first()->total_vote_value ?? 0 }}</p>
                </div>

                @else
                    <div class="absolute flex justify-center  items-center gap-2 bg-green-500 px-3 py-1 rounded text-white right-4 top-4">
                        <i class="fa-solid fa-medal"></i>
                        <p class="font-semibold">{{ $submission->votes->first()->total_vote_value ?? 0 }}</p>
                    </div>
                @endif
            </div>

            @if (!$submission->alreadyVoted)
            <hr class="text-gray-500">
            <div class="px-4 mb-4 mt-4">
                <form action="{{ route('vote#submit', $submission->id) }}" method="POST">
                    @csrf
                    <div>
                        <label class="font-medium " for="vote_value">Vote Range (1-10):</label>
                        <p class="font-regular text-gray-500 text-sm mb-4">This vote gives a score for your friend's work. Make sure you have checked the submission file before voting</p>

                        <input class="mb-2 block w-full py-2 pl-4 pr-4 text-black placeholder-gray-500 transition-all duration-200 border border-gray-200 rounded-md bg-gray-50 focus:outline-none focus:bg-white" type="number" name="vote_value" min="1" max="10" required>
                    </div>
                    <button class="w-full py-2 rounded bg-red-500 text-white" type="submit">Vote</button>
                </form>
            </div>
            @else
            <div class="px-4">
                <button class="w-full py-2 rounded bg-green-500 text-white mb-4" type="disable" disabled>Assignment Already Voted</button>
            </div>
                
            @endif

        </div>
    @endforeach
    </div>

@endforeach
@endif




{{-- @if ($submissions->isEmpty())
<div class="flex items-center justify-center h-screen">
    <div class="grid gap-4 w-60">
        <svg class="mx-auto" xmlns="http://www.w3.org/2000/svg" width="116" height="121" viewBox="0 0 116 121" fill="none">
        <path d="M0.206909 63.57C0.206909 31.7659 25.987 6.12817 57.6487 6.12817C89.2631 6.12817 115.079 31.7541 115.079 63.57C115.079 77.0648 110.43 89.4805 102.627 99.2755C91.8719 112.853 75.4363 121 57.6487 121C39.7426 121 23.4018 112.794 12.6582 99.2755C4.85538 89.4805 0.206909 77.0648 0.206909 63.57Z" fill="#EEF2FF" />
        <path d="M72.7942 0.600875L72.7942 0.600762L72.7836 0.599331C72.3256 0.537722 71.8622 0.5 71.3948 0.5H22.1643C17.1256 0.5 13.0403 4.56385 13.0403 9.58544V107.286C13.0403 112.308 17.1256 116.372 22.1643 116.372H93.1214C98.1725 116.372 102.245 112.308 102.245 107.286V29.4482C102.245 28.7591 102.17 28.0815 102.019 27.4162L102.019 27.416C101.615 25.6459 100.67 24.0014 99.2941 22.7574C99.2939 22.7572 99.2937 22.757 99.2934 22.7568L77.5462 2.89705C77.5461 2.89692 77.5459 2.89679 77.5458 2.89665C76.2103 1.66765 74.5591 0.876968 72.7942 0.600875Z" fill="white" stroke="#E5E7EB" />
        <circle cx="60.2069" cy="61" r="21.0256" fill="#EEF2FF" />
        <path d="M74.6786 46.1412L74.6783 46.1409C66.5737 38.0485 53.4531 38.0481 45.36 46.1412C37.2552 54.2341 37.2551 67.3666 45.3597 75.4596C53.4529 83.5649 66.5739 83.5645 74.6786 75.4599C82.7716 67.3669 82.7716 54.2342 74.6786 46.1412ZM79.4694 41.3508C90.2101 52.0918 90.2101 69.5093 79.4694 80.2502C68.7166 90.9914 51.3104 90.9915 40.5576 80.2504C29.8166 69.5095 29.8166 52.0916 40.5576 41.3506C51.3104 30.6096 68.7166 30.6097 79.4694 41.3508Z" stroke="#E5E7EB" />
        <path d="M83.2471 89.5237L76.8609 83.1309C78.9391 81.5058 80.8156 79.6106 82.345 77.6546L88.7306 84.0468L83.2471 89.5237Z" stroke="#E5E7EB" />
        <path d="M104.591 94.4971L104.59 94.4969L92.7346 82.653C92.7342 82.6525 92.7337 82.652 92.7332 82.6515C91.6965 81.6018 90.0076 81.6058 88.9629 82.6505L89.3089 82.9965L88.9629 82.6505L81.8573 89.7561C80.8213 90.7921 80.8248 92.4783 81.8549 93.5229L81.8573 93.5253L93.7157 105.384C96.713 108.381 101.593 108.381 104.591 105.384C107.6 102.375 107.6 97.5062 104.591 94.4971Z" fill="#A5B4FC" stroke="#818CF8" />
        <path d="M62.5493 65.6714C62.0645 65.6714 61.6626 65.2694 61.6626 64.7729C61.6626 62.7866 58.6595 62.7866 58.6595 64.7729C58.6595 65.2694 58.2576 65.6714 57.761 65.6714C57.2762 65.6714 56.8743 65.2694 56.8743 64.7729C56.8743 60.422 63.4478 60.4338 63.4478 64.7729C63.4478 65.2694 63.0458 65.6714 62.5493 65.6714Z" fill="#4F46E5" />
        <path d="M70.1752 58.0694H66.4628C65.9662 58.0694 65.5642 57.6675 65.5642 57.1709C65.5642 56.6862 65.9662 56.2842 66.4628 56.2842H70.1752C70.6717 56.2842 71.0737 56.6862 71.0737 57.1709C71.0737 57.6675 70.6717 58.0694 70.1752 58.0694Z" fill="#4F46E5" />
        <path d="M53.8596 58.0693H50.1472C49.6506 58.0693 49.2487 57.6673 49.2487 57.1708C49.2487 56.686 49.6506 56.2841 50.1472 56.2841H53.8596C54.3443 56.2841 54.7463 56.686 54.7463 57.1708C54.7463 57.6673 54.3443 58.0693 53.8596 58.0693Z" fill="#4F46E5" />
        <rect x="28.9248" y="16.3846" width="30.7692" height="2.05128" rx="1.02564" fill="#4F46E5" />
        <rect x="28.9248" y="100.487" width="41.0256" height="4.10256" rx="2.05128" fill="#A5B4FC" />
        <rect x="28.9248" y="22.5385" width="10.2564" height="2.05128" rx="1.02564" fill="#4F46E5" />
        <circle cx="42.2582" cy="23.5641" r="1.02564" fill="#4F46E5" />
        <circle cx="46.3607" cy="23.5641" r="1.02564" fill="#4F46E5" />
        <circle cx="50.4633" cy="23.5641" r="1.02564" fill="#4F46E5" />
        </svg>
        <div>
        <h2 class="text-center text-black text-base font-semibold leading-relaxed pb-1">No assignment available to vote</h2>
        <p class="text-center text-gray-600 text-sm font-normal leading-snug pb-4">Please wait until your batchmates <br> upload their assignments.</p>
        <div class="flex gap-3">
        </div>
        </div>
    </div>
</div>
@else


<div class="container mb-4 mt-4">
    <h1 class="text-xl font-bold mb-4">Assignment Gallery</h1>
    <hr>
</div>

@foreach ($submissions as $courseName => $submissionGroup)
    <div class="container mb-4">
        <h2 class="text-lg font-semibold mb-2">{{ $courseName }}</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 sm:grid-cols-2 gap-3 mb-4">
            @foreach ($submissionGroup as $submission)
                <div class="relative flex flex-col bg-white shadow-sm border border-slate-200 rounded-lg w-auto">
                    <a href="{{ asset($submission->submission_file) }}" target="_target">
                        <div class="relative h-56 m-2.5 overflow-hidden text-white rounded-md">
                            <img src="https://images.unsplash.com/photo-1540553016722-983e48a2cd10?ixlib=rb-1.2.1&amp;ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&amp;auto=format&amp;fit=crop&amp;w=800&amp;q=80" alt="card-image" />
                        </div>
                    </a>
                    <div class="flex px-4 mb-4">
                        <div class="w-full">
                            <h6 class="mb-0 text-slate-800 text-xl font-semibold">
                                {{$submission->course_name}}
                            </h6>
                            <div class="flex items-center">
                                <div class="text-sm">
                                    <a href="#" class="text-gray-900 font-semibold leading-none hover:text-indigo-600">{{$submission->user->name}}</a>
                                </div>
                            </div>
                        </div>

                        <div>
                            <i class="fa-solid fa-medal"></i>
                            <p>{{ $submission->votes->first()->total_vote_value ?? 0 }}</p>
                        </div>
                    </div>

                    @if (!$submission->alreadyVoted)
                    <div class="px-4 mb-4">
                        <form action="{{ route('vote#submit', $submission->id) }}" method="POST">
                            @csrf
                            <div>
                                <label for="vote_value">Nilai (1-10):</label>
                                <input class="mb-2 block w-full py-2 pl-4 pr-4 text-black placeholder-gray-500 transition-all duration-200 border border-gray-200 rounded-md bg-gray-50 focus:outline-none focus:bg-white" type="number" name="vote_value" min="1" max="10" required>
                            </div>
                            <button class="w-full py-2 rounded bg-red-500 text-white" type="submit">Vote</button>
                        </form>
                    </div>
                    @else
                        <p class="flex w-full text-center text-gray-500 px-4 mb-4">You have already voted on this assignment.</p>
                    @endif

                </div>
            @endforeach
        </div>
    </div>
@endforeach --}}


{{-- <div class="container grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 sm:grid-cols-2 gap-3 mb-4">
    @foreach ($submissions as $submission)
        <div class=" relative flex flex-col bg-white shadow-sm border border-slate-200 rounded-lg w-auto">
            <a href="{{ asset($submission->submission_file) }}" target="_target">
            <div class="relative h-56 m-2.5 overflow-hidden text-white rounded-md">
                <img src="https://images.unsplash.com/photo-1540553016722-983e48a2cd10?ixlib=rb-1.2.1&amp;ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&amp;auto=format&amp;fit=crop&amp;w=800&amp;q=80" alt="card-image" />
            </div>
            </a>
            <div class="flex px-4 mb-4 ">
                <div class=" w-full">
                    <h6 class="mb-0 text-slate-800 text-xl font-semibold">
                        {{$submission->course_name}}
                    </h6>
                    <div class="flex items-center">
                        <div class="text-sm">
                            <a href="#" class="text-gray-900 font-semibold leading-none hover:text-indigo-600">{{$submission->user->name}}</a>
                        </div>
                    </div>
                </div>

                <div>
                    <i class="fa-solid fa-medal"></i>
                    <p>{{ $submission->votes->first()->total_vote_value ?? 0 }}</p>
                </div>
            </div>

            @if (!$submission->alreadyVoted)
            <div class="px-4 mb-4">
                <form action="{{ route('vote#submit', $submission->id) }}" method="POST">
                    @csrf
                    <div>
                        <label for="vote_value">Nilai (1-10):</label>
                        <input class="mb-2 block w-full py-2 pl-4 pr-4 text-black placeholder-gray-500 transition-all duration-200 border border-gray-200 rounded-md bg-gray-50 focus:outline focus:caret-black-600 focus:bg-white caret-black-600" type="number" name="vote_value" min="1" max="10" required>
                    </div>
                    <button class="w-full py-2 rounded bg-red-500 text-white" type="submit">Vote</button>
                </form>
            </div>
            @else
                <p class="flex w-full text-center text-gray-500 px-4 mb-4">You already vote this assignment.</p>
            @endif

        </div>
    </a>
    @endforeach

</div> --}}
{{-- @endif --}}

@endsection
